...
impl From<ckb_jsonrpc_types::Script> for AddressPayload {
    #[allow(clippy::fallible_impl_from)]
    fn from(lock: ckb_jsonrpc_types::Script) -> AddressPayload {
        let hash_type: ScriptHashType = lock.hash_type.try_into().expect("Invalid hash_type");

        let code_hash_h256: H256 = lock.code_hash.clone();
        let code_hash = lock.code_hash.pack();
        let args: Bytes = Bytes::copy_from_slice(lock.args.as_bytes());
        if hash_type == ScriptHashType::Type
            && code_hash_h256 == SIGHASH_TYPE_HASH
            && args.len() == 20
        {
